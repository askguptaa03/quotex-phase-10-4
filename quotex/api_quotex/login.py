...
# NOTE (compat fix): anchor to the quotex/ package directory itself, matching
# Config()'s default in config.py. Previously this was Path("sessions")
# (relative to the current working directory), which pointed at a different
# location than the session.json that market_analyzer/fetch_data.py reads
# (quotex/session.json) and that already exists on disk. Both now agree.
SESSION_DIR = Path(__file__).resolve().parent.parent
SESSION_FILE = SESSION_DIR / "session.json"
CREDENTIALS_FILE = SESSION_DIR / "config.json"
QX_BASE = "https://qxbroker.com"

# Known-invalid SSID sentinel values seen in the wild (e.g. a JS bug that
# stringifies the whole `localStorage`/`sessionStorage` object instead of
# reading a specific key). These are rejected immediately without spending
# a network round-trip on websocket validation.
_INVALID_SSID_VALUES = {
    "[object storage]",
    "[object object]",
    "null",
    "undefined",
...
def _looks_like_valid_ssid(candidate: Optional[str]) -> bool:
    """Cheap sanity check before we bother opening a WebSocket to validate."""
    if not candidate or not isinstance(candidate, str):
        return False
    stripped = candidate.strip()
    if not stripped:
        return False
    if stripped.lower() in _INVALID_SSID_VALUES:
        return False
    if len(stripped) < 10:
        return False
    return True
...
async def get_ssid(email: str = None, password: str = None, email_pass: str = None, lang: str = "en", is_demo: bool = True, keep_browser_on_error: bool = False) -> Tuple[bool, Dict]:
    ...
    if session_data and SESSION_FILE.exists():
        ...
        if not _looks_like_valid_ssid(ssid_candidate):
            logger.warning(
                f"Existing SSID fails sanity check (value: {ssid_candidate!r}); "
                "skipping network validation and re-authenticating."
            )
            ssid_candidate = None
