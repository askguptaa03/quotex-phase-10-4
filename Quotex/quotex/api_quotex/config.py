...
class Config:
    ...
    def __init__(self, resource_path: Optional[str] = None):
        ...
        if resource_path is None:
            # Default to the `quotex/` package root (parent of api_quotex/),
            # NOT a "sessions" folder relative to the current working
            # directory. This fixes a real bug: previously session.json /
            # config.json were written to "<cwd>/sessions/..." while
            # market_analyzer/config.py (and quotex/session.json itself,
            # which already exists on disk) expect "quotex/session.json".
            # Anchoring to this file's location makes it work no matter
            # where the script is invoked from, and keeps session.json
            # compatible with the existing file layout.
            resource_path = str(Path(__file__).resolve().parent.parent)
    ...
    def _load_session(self) -> None:
        with self._lock:
            ...
            # Check env var before touching session.json so the app works
            # in Replit Secrets / deployment env without any local file.
            # SAFETY: only log the length, never the value.
            _env_ssid = os.environ.get("QUOTEX_SSID", "").strip()
            if _env_ssid and len(_env_ssid) >= 10 and _env_ssid != "[object Storage]":
                self.session_data["ssid"] = _env_ssid
                self._session_loaded = True
                logger.info(f"SSID loaded from QUOTEX_SSID env var "
                            f"(length: {len(_env_ssid)})")
                return
