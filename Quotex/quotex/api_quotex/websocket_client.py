...
import websockets
# NOTE (compat fix): as of websockets>=14, the top-level `websockets.connect`
# alias points at the NEW asyncio implementation, which renamed
# `extra_headers` -> `additional_headers` and removed the `.closed` /
# `.open` properties that this client relies on everywhere below.
# We explicitly pin to the legacy client implementation here so the rest of
# this file (which reads `.closed`, passes `extra_headers`, etc.) keeps
# working unchanged, without altering any Socket.IO/Engine.IO protocol
# logic. The legacy implementation is still shipped and supported by the
# `websockets` package (deprecated but maintained) as of 16.0.
try:
    from websockets.legacy.client import connect as _ws_connect
    from websockets.legacy.client import WebSocketClientProtocol as _WSClientProtocol
except ImportError:
    # Very old websockets (<10) had everything at the top level.
    _ws_connect = websockets.connect
    _WSClientProtocol = websockets.WebSocketClientProtocol
...
class ConnectionPool:
    ...
    def __init__(self, max_connections: int = 3):
        ...
        self.active_connections: Dict[str, _WSClientProtocol] = {}
...
class AsyncWebSocketClient:
    ...
    def __init__(self) -> None:
        self.websocket: Optional[_WSClientProtocol] = None
    ...
    async def connect(self, urls: List[str], ssid: str) -> bool:
        for url in urls:
            try:
                ...
                # We use the legacy client (imported as _ws_connect above),
                # which has accepted `extra_headers` since websockets 4.0 and
                # still does in 16.x, so no version branching is needed here.
                connect_kwargs = {
                    "uri": url,
                    "ssl": ssl_context,
                    "ping_interval": 25,
    ...
    async def _send_handshake(self, ssid: str) -> None:
        try:
            ...
            if auth_response.startswith('42["s_authorization"') or auth_response.startswith('451-["instruments/list"'):
                ...
                # Mark as authenticated BEFORE emitting the event so that any
                # caller polling getattr(ws, 'is_authenticated') sees True
                # immediately (Fix 1 — was causing _wait_for_authentication to
                # always spin for the full 30-second timeout).
